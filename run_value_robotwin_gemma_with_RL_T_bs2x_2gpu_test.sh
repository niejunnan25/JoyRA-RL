#!/usr/bin/env bash
# GemmaValue 2-GPU test training for RobotWin.
# Usage:
#   bash run_value_robotwin_gemma_with_RL_T_bs2x_2gpu_test.sh
#   bash run_value_robotwin_gemma_with_RL_T_bs2x_2gpu_test.sh <DATA_MIX>
set -euo pipefail
...
RUN_SUFFIX="gemma270_siglip2_2gpu_bs2x_neg600_hplusneg_$(date +%H%M%S)"
...
# Batch size is per GPU/rank under DDP. With 2 GPUs, BATCH_SIZE=32 means global batch size 64.
BATCH_SIZE=32
# Keep the same optimizer-step budget as the 8-GPU bs2x run:
# max_train_steps=50000 optimizer updates.
# This matches update count, not total samples. Total samples are 1/4 of the
# 8-GPU run when per-GPU batch size stays at 32.
