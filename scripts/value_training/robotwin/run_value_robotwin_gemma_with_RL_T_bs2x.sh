...
setup_value_training_distributed 29520 8
...
DATA_MIX="${1:-${DATA_MIX:-robotwin_orig_plus_offline_v2}}"

OUTPUTS_BASE="${PROJECT_ROOT}/outputs"
DATE_STR=$(date +%Y%m%d)
mkdir -p "${OUTPUTS_BASE}"
RUN_SUFFIX=${RUN_SUFFIX:-"gemma270_siglip2_bs2x_neg600_hplusneg_$(date +%H%M%S)"}
