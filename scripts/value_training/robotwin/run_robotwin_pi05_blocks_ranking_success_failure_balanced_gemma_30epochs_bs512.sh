...
# Frame-balanced block-ranking value training:
#   blocks_ranking_rgb_success_failure_balanced:  204,523 frames
#   blocks_ranking_size_success_failure_balanced:  62,357 frames
#   total: 266,880 frames, success/failure ~= 50/50 by frame count
#
# 8 GPUs x batch_size 64 = global batch size 512.
# 30 traditional epochs over train_split=0.9:
#   266,880 * 0.9 * 30 / 512 ~= 14,074 optimizer steps.
export DATA_ROOT_DIR=${DATA_ROOT_DIR:-/mnt/workspace/users/niejunnan/datasets/robotwin_rollout_lerobot}
export DATA_MIX=${DATA_MIX:-robotwin_pi05_blocks_ranking_success_failure_balanced}
export BIG_NEGATIVE=${BIG_NEGATIVE:-600}
export BATCH_SIZE=${BATCH_SIZE:-64}
export MAX_TRAIN_STEPS=${MAX_TRAIN_STEPS:-14074}
...
export RUN_SUFFIX=${RUN_SUFFIX:-"gemma270_siglip2_robotwin_pi05_blocks_ranking_success_failure_balanced_30epochs_bs512_neg${BIG_NEGATIVE}_$(date +%H%M%S)"}
