#!/usr/bin/env bash
# Agibot 价值函数训练（DATA_MIX 外参 + outputs 目录）。用法:
#   bash run_value_agibot_with_RL_T.sh <DATA_MIX>
# 预训练权重（run_value_pretrain_with_RL_T.sh agi_suqian_egodex_robocasa_robotwin_mix 等）:
...
# agi_suqian_egodex_robocasa_robotwin_mix 预训练 checkpoint；置空则仅加载 VLM 底座
INIT_CHECKPOINT="${INIT_CHECKPOINT:-/mnt/workspace1/users/tangyili/Projects/JoyRA-RL/outputs/value/agi_suqian_egodex_robocasa_robotwin_mix_20260413/checkpoint_step_1000000.pt}"
...
# 时序降采样步长（与 run_value_pretrain_with_RL_T.sh 对齐）
