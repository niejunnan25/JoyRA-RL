...
class VideoTransform(ModalityTransform):
    ...
    def set_metadata(self, dataset_metadata: DatasetMetadata):
        ...
        # 针对多数据集混合的情况，不是所有数据集都一定具备所有视角。
        # 这里对当前数据集缺失的视角做「降级处理」：打印 warning 并从 apply_to 中移除该视角，
        # 而不是直接抛异常。这可以让「多视角配置 + 单视角数据集」正常工作。
        new_apply_to: list[str] = []
        for key in list(self.apply_to):
            ...
            if sub_key in dataset_metadata.modalities.video:
                ...
                # 视角在该数据集不存在，跳过并给出提示
                print(
                    f"[WARN] VideoTransform: video key '{sub_key}' not found in dataset metadata "
                    f"for apply_to='{key}'. Available keys: {list(dataset_metadata.modalities.video.keys())}. "
                    "This view will be ignored for this dataset."
                )
                continue

            new_apply_to.append(key)

        # 如果所有请求视角都在当前数据集中缺失，则做一次“回退”：
        # 优先选择 canonical 的 `image_head_color`，否则选择任意一个可用视角，
        # 确保至少有一个视觉输入，而不是直接报错终止训练。
        if not new_apply_to:
            available_video_keys = list(dataset_metadata.modalities.video.keys())
            if not available_video_keys:
                # 这个数据集完全没有 video 模态，仍然抛异常
                raise ValueError(
                    "VideoTransform: none of the requested video views exist in dataset metadata, "
                    "and dataset has no available video keys at all. "
                    f"Requested views (apply_to): {self.apply_to}."
                )

            # 优先用 image_head_color 作为退化视角，否则就拿第一个可用视角
            fallback_key = (
                "image_head_color"
                if "image_head_color" in available_video_keys
                else available_video_keys[0]
            )
            print(
                "[WARN] VideoTransform: none of the requested video views exist for this dataset. "
                f"Requested views (apply_to): {self.apply_to}. "
                f"Available video keys: {available_video_keys}. "
                f"Falling back to use '{fallback_key}' as the only view."
            )
            key = f"video.{fallback_key}"
            self.original_resolutions[key] = dataset_metadata.modalities.video[
                fallback_key
            ].resolution
            new_apply_to = [key]
...
class VideoResizePad(VideoTransform):
    size: int = Field(..., description="The target size (both height and width will be this value)")
    ...
    fill_value: float = Field(default=0.0, description="The value to fill the padding (0.0 for black)")
    ...
    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable:
        """Get the resize and pad transform that keeps aspect ratio.

        Args:
            mode (Literal["train", "eval"]): The mode to get the transform for.

        Returns:
            Callable: The resize and pad transform that resizes the longest edge to target size and pads the rest.
        """
        if self.backend == "torchvision":
            return self._torchvision_resize_pad
        elif self.backend == "albumentations":
            # For albumentations, use LongestMaxSize + PadIfNeeded
            interpolation = self._get_interpolation(self.interpolation, self.backend)
            return A.Compose([
                A.LongestMaxSize(max_size=self.size, interpolation=interpolation, p=1),
                A.PadIfNeeded(
                    min_height=self.size,
                    min_width=self.size,
                    border_mode=cv2.BORDER_CONSTANT,
                    value=int(self.fill_value * 255),  # Convert to uint8 range for numpy arrays
                    position=A.PadIfNeeded.PositionType.CENTER,  # Center the image
                    p=1
                )
            ])
        else:
    ...
    def _torchvision_resize_pad(self, frames: torch.Tensor) -> torch.Tensor:
        """Resize and pad frames using torchvision.
        
        Args:
            frames: tensor of shape [N, C, H, W] where N could be batch*time
        
        Returns:
            tensor of shape [N, C, target_size, target_size]
        """
        interpolation = self._get_interpolation(self.interpolation, self.backend)
