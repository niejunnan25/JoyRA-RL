...
from starVLA.dataloader.gr00t_lerobot.relative_action_stats.lerobot_episode_loader import LeRobotEpisodeLoader
from starVLA.dataloader.gr00t_lerobot.state_action.action_chunking import EndEffectorActionChunk, JointActionChunk
from starVLA.dataloader.gr00t_lerobot.state_action.pose import EndEffectorPose, JointPose
from starVLA.dataloader.gr00t_lerobot.relative_action_stats.types import ActionFormat, ActionRepresentation, ActionType, ModalityConfig
from starVLA.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from starVLA.dataloader.gr00t_lerobot.relative_action_stats.utils import to_json_serializable
...
class RelativeActionLoader:
    ...
    def load_relative_actions(self, trajectory_id: int) -> list[np.ndarray]:
        ...
        for i in range(usable_length):
            ...
            # Handle object dtype arrays (where each element is itself an array)
            if isinstance(last_state, np.ndarray) and last_state.dtype == object:
                last_state = np.array(last_state)
            if actions.dtype == object:
                # Flatten and stack the nested arrays
                if actions.ndim == 2:
                    # Shape is (1, n) or (m, n), flatten to get all elements
                    actions = np.stack([a for a in actions.flat if isinstance(a, np.ndarray)])
                elif actions.ndim == 1:
                    # Shape is (n,), each element is an array
                    actions = np.stack([a for a in actions if isinstance(a, np.ndarray)])
            if self.action_config.type == ActionType.EEF:
                # Determine rotation type based on action format
                if self.action_config.format == ActionFormat.XYZ_ROT6D:
                    # 9D: xyz (3) + rot6d (6)
                    assert len(last_state) == 9, f"Expected 9D state (xyz+rot6d), got {len(last_state)}D"
                    assert actions.shape[1] == 9, f"Expected 9D action (xyz+rot6d), got {actions.shape[1]}D"
                    rotation_type = "rot6d"
                    translation_dim = 3
                elif self.action_config.format == ActionFormat.XYZ_ROTVEC:
                    # 6D: xyz (3) + rotvec/axis-angle (3)
                    assert len(last_state) == 6, f"Expected 6D state (xyz+rotvec), got {len(last_state)}D"
                    assert actions.shape[1] == 6, f"Expected 6D action (xyz+rotvec), got {actions.shape[1]}D"
                    rotation_type = "rotvec"
                    translation_dim = 3
                else:
                    raise ValueError(f"Unsupported action format: {self.action_config.format}")

                reference_frame = EndEffectorPose(
                    translation=last_state[:translation_dim],
                    rotation=last_state[translation_dim:],
...
def generate_rel_stats(dataset_path: Path | str, embodiment_tag: EmbodimentTag) -> None:
    ...
    if stats_path.exists():
        with open(stats_path, "r") as f:
            stats = json.load(f)
        print(f"Loaded relative stats from {stats_path}")
        return stats
    else:
        stats = {}
