...
class FlowmatchingActionHead(nn.Module):
    ...
    def forward(self, vl_embs: torch.Tensor, actions: torch.Tensor, state: torch.Tensor = None, encoder_attention_mask=None, action_mask=None):
        """
    ...
    def predict_action(self, vl_embs: torch.Tensor, state: torch.Tensor = None) -> torch.Tensor:
        ...
        state_features = self.state_encoder(state) if state is not None else None
        # 只在第一步注册 hooks 并分析
        analyze_attention = state_features is not None
