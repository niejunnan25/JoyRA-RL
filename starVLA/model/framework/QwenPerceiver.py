...
@FRAMEWORK_REGISTRY.register("QwenPerceiver")
class Qwen_Perceiver(baseframework):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #     action_head_cfg = TimeAwareActionHead_Config()
        #     time_aware_action_head_config = action_head_cfg.convert_to_dict()
        #     time_aware_action_head_config['action_dim'] = 1536
        #     time_aware_action_head_config['layers'] = 16
        #     time_aware_action_head_config['heads'] = 16
        #     act_config = {}
        #     act_config['diffusion_model_cfg'] =  time_aware_action_head_config
        #     act_config["tune_projector"] = True
        #     act_config["tune_diffusion_model"] = True
        #     act_config["use_time_aware_action_head"] = True
        #     action_model_params = dict(self.config.framework.action_model)
        #     # 2. 将这些参数合并到 act_config 中
        #     act_config.update(action_model_params)
        #     action_head_config = FlowmatchingActionHeadConfig(**act_config)
        #     # import ipdb;ipdb.set_trace()
        #     self.action_head = FlowmatchingActionHead(action_head_config)
        #     # self.action_model = TimeAwareActionHead(config=time_aware_action_head_config) 
        # else:
        #     self.action_model: TimeAwareActionHead = get_action_model(config=self.config)  # 修复后续引用
    ...
    def forward(
        ...
        **kwargs,
        ...
        action_mask = [example["action_mask"] for example in examples] if "action_mask" in examples[0] else None  # [B, len, action_dim]
...
if __name__ == "__main__":
    ...
    model: Qwen_Perceiver = Qwen_Perceiver(cfg)
    print(model)
