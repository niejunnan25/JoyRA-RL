...
# 只使用 robotwin_aloha_agilex_550_mix 数据集
...
# Bin range 配置（仅在 NORMALIZE_RETURNS=false 时生效，可保留为空使用数据驱动模式）
#BIN_MIN=-1200.0
#BIN_MAX=0.0
...
# 模型保存目录（robotwin 专用）
OUTPUT_DIR=/mnt/workspace/users/daiyixiang/JoyRA-RL/outputs_value_robotwin_aloha_agilex_550_mix

# Return 缓存目录（robotwin 专用）
RETURNS_CACHE_DIR=/mnt/workspace/users/daiyixiang/JoyRA-RL/returns_cache_robotwin_aloha_agilex_550_mix
