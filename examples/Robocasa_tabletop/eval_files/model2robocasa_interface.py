...
class PolicyWarper:
    ...
    def unnormalize_actions(normalized_actions: np.ndarray, action_norm_stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        normalized_actions_dim = normalized_actions.shape[-1]
        if len(mask) < normalized_actions_dim:
            # 只保留有效动作维度
            valid_action_dim = len(mask)
            # 取normalized_actions的前valid_action_dim维度
            normalized_actions = normalized_actions[:, :, :valid_action_dim]
        action_high, action_low = np.array(action_norm_stats["max"]), np.array(action_norm_stats["min"])

        normalized_actions = np.clip(normalized_actions, -1, 1)[..., :29]
    ...
    #注意修改_resize_image，这个取决于训练时有没有使用VideoResizePad，若使用，则使用第二个_resize_image
    ...
    def _resize_image(self, image: np.ndarray) -> np.ndarray:
        """
        Resize image with padding to keep aspect ratio, matching VideoResizePad behavior.
        ...
        """
        H, W = image.shape[:2]
        target_size = self.image_size[0]     # Assuming square target size
        ...
        # Pad with black (0) to match VideoResizePad's default fill_value=0.0
    ...
    def visualize_epoch(self, predicted_raw_actions: Sequence[np.ndarray], images: Sequence[np.ndarray], save_path: str) -> None:
        ...
        pred_actions = np.array(
            [np.concatenate([a["world_vector"], a["rotation_delta"], a["open_gripper"]], axis=-1) for a in predicted_raw_actions])
    ...
    def _check_unnorm_key(norm_stats, unnorm_key):
        ...
        if unnorm_key is None:
            assert len(norm_stats) == 1, (f"Your model was trained on more than one dataset, "
                                          f"please pass a `unnorm_key` from the following options to choose the statistics "
                                          f"used for un-normalizing actions: {norm_stats.keys()}")
            unnorm_key = next(iter(norm_stats.keys()))

        assert unnorm_key in norm_stats, (f"The `unnorm_key` you chose is not in the set of available dataset statistics, "
